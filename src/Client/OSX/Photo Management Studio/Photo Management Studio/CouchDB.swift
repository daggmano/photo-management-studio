...
public class CouchDB {
    ...
    public func list(done: (ListResponse) -> Void) {
        ...
        HTTP.get("\(self.url)_all_dbs", headers: self.headers) { response in
            switch response {
            case .Error(let error):
                done(.Error(error))
            case .Success(let json, let res):
                ...
                done(.Success(HTTPListResponse(data: json)))
            }
        }
    }
    ...
    public func replicate(request: ReplicationRequest, done: (ReplicateResponse) -> Void) {
        ...
        HTTP.post("\(self.url)_replicate", headers: self.headers, data: data) { result in
            switch result {
            case .Error(let error):
                done(.Error(error))
            case .Success(let json, let response):
                ...
                done(.Success(HTTPReplicationResponse(data: json)))
            }
        }
    }
